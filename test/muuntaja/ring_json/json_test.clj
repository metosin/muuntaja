...
(deftest test-json-body
  (let [handler (muuntaja/wrap-format +handler+ (-> muuntaja/default-options
  ...
  (testing "custom malformed json"
    (let [malformed {:status 400
          ...
          handler (muuntaja/wrap-format +handler+ (-> muuntaja/default-options
                                                      muuntaja/no-encoding
                                                      (assoc :handle-error (constantly
                                                                             malformed))))
  ...
  #_(let [handler  (fn [_] {:status 200 :headers {} :body {:bigdecimals cheshire.parse/*use-bigdecimals?*}})]
