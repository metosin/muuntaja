...
(def json-echo
  (wrap-api-params identity {:formats [:json]}))
...
(deftest json-options-test
  (is (= {:foo-bar "bar"}
         (:body-params ((wrap-api-params identity {:formats [:json-kw]
                                                   :format-options {:json-kw {:key-fn key-fn}}})
                         {:content-type "application/json"
                          :body (stream "{\"foo_bar\":\"bar\"}")}))))
  (is (= {:foo-bar "bar"}
         (:body-params ((wrap-api-params identity {:format-options {:json {:key-fn key-fn}}})
                         {:content-type "application/json"
                          :body (stream "{\"foo_bar\":\"bar\"}")})))))

(def yaml-echo
  (wrap-api-params identity {:formats [:yaml]}))
...
(def yaml-kw-echo
  (wrap-api-params identity {:formats [:yaml-kw]}))
...
(def msgpack-echo
  (wrap-api-params identity {:formats [:msgpack]}))
...
(def msgpack-kw-echo
  (wrap-api-params identity {:formats [:msgpack-kw]}))
...
(def clojure-echo
  (wrap-api-params identity {:formats [:edn]}))
...
(def transit-json-echo
  (wrap-api-params identity {:formats [:transit-json]}))
...
(def transit-msgpack-echo
  (wrap-api-params identity {:formats [:transit-msgpack]}))
...
(def custom-transit-json-echo
  (wrap-api-params identity {:format-options {:transit-json {:handlers readers}}}))

(def transit-body "[\"^ \", \"~:p\", [\"~#Point\",[1,2]]]")

(deftest read-custom-transit
  (testing "wrap-api-params, transit options"
    (let [req (custom-transit-json-echo {:content-type "application/transit+json"
